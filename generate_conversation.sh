...
#paralinguistic (laugh/cough) 数据路径
...
echo "Paralinguistic dir: $PARALINGUISTIC_DIR"
echo "Laugh probability: $LAUGH_PROB"
echo "Cough probability: $COUGH_PROB"
echo "=========================================="
...
python generate_conversation_definitions.py \
    ...
    --output-dir $OUTPUT_DIR/train0/conversations \
    --sound-events-dir $SOUND_events_dir \
    --paralinguistic-dir $PARALINGUISTIC_DIR \
    --num-conversations $TRAIN_CONVS \
    --duration $CONV_DURATION \
    --num-speakers $NUM_SPEAKERS \
    --sampling-rate $SAMPLING_RATE \
    --laugh-prob $LAUGH_PROB \
    --cough-prob $COUGH_PROB \
    --seed 42
...
python generate_conversation_definitions.py \
    ...
    --output-dir $OUTPUT_DIR/eval0/conversations \
    --sound-events-dir $SOUND_events_dir \
    --paralinguistic-dir $PARALINGUISTIC_DIR \
...
python generate_conversation_definitions.py \
    --kaldi-data-dir ./Kaldidatadir/dev_clean_persession \
    --output-dir $OUTPUT_DIR/test0/conversations \
    --sound-events-dir $SOUND_events_dir \
    --paralinguistic-dir $PARALINGUISTIC_DIR \
...
python synthesize_conversations.py \
    --conv-list $OUTPUT_DIR/train0/conversations/conversations.list \
    --conv-dir $OUTPUT_DIR/train0/conversations \
    --wav-scp ./Kaldidatadir/alltrain_persession/wav.scp \
    --output-dir $OUTPUT_DIR/train0/audio \
    --sampling-rate $SAMPLING_RATE \
    --num-workers 16

# # 评估集
python synthesize_conversations.py \
    --conv-list $OUTPUT_DIR/eval0/conversations/conversations.list \
    --conv-dir $OUTPUT_DIR/eval0/conversations \
    --wav-scp ./Kaldidatadir/dev_clean_persession/wav.scp \
    --output-dir $OUTPUT_DIR/eval0/audio \
    --sampling-rate $SAMPLING_RATE \
    --num-workers 8

# 测试集
python synthesize_conversations.py \
    --conv-list $OUTPUT_DIR/test0/conversations/conversations.list \
    --conv-dir $OUTPUT_DIR/test0/conversations \
    --wav-scp ./Kaldidatadir/dev_clean_persession/wav.scp \
    --output-dir $OUTPUT_DIR/test0/audio \
    --sampling-rate $SAMPLING_RATE \
    --num-workers 8
...
python generate_conversation_labels.py \
        --conv-list $OUTPUT_DIR/train0/conversations/conversations.list \
        --conv-dir $OUTPUT_DIR/train0/conversations \
        --output-dir $OUTPUT_DIR/train0/labels \
        --duration $CONV_DURATION \
...
python generate_conversation_labels.py \
        --conv-list $OUTPUT_DIR/eval0/conversations/conversations.list \
        --conv-dir $OUTPUT_DIR/eval0/conversations \
        --output-dir $OUTPUT_DIR/eval0/labels \
        --duration $CONV_DURATION \
        --frame-rate $FRAME_RATE
...
python generate_conversation_labels.py \
        --conv-list $OUTPUT_DIR/test0/conversations/conversations.list \
        --conv-dir $OUTPUT_DIR/test0/conversations \
        --output-dir $OUTPUT_DIR/test0/labels \
        --duration $CONV_DURATION \
        --frame-rate $FRAME_RATE
